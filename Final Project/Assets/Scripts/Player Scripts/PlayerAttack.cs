...
public class PlayerAttack : MonoBehaviour
{
    ...
    void ComboAttacks() 
    {
        ...
        if (Input.GetKeyDown(KeyCode.X))
        {
            ...
            // return will exit because there will be no more combos to perform
            if (current_Combo_State == ComboState.KICK_2 ||
                current_Combo_State == ComboState.PUNCH_3)
                return;
            // if the current combo is none or pucnh1 or 2
            // then the current combo state will go to kick 1 to chain combo
